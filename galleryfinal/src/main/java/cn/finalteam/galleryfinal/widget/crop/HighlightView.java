...
class HighlightView {
    ...
    public HighlightView(View view, int highlightColor,boolean isCircle) {
        viewContext = view;
    ...
    protected void draw(Canvas canvas) {
        ...
        if (!hasFocus()) {
            ...
            //透明边框
            //四边形
            thirdPath.addRect(new RectF(drawRect), Path.Direction.CW);
            circlePath.addCircle((drawRect.right + drawRect.left) / 2, (drawRect.bottom + drawRect.top) / 2, (drawRect.bottom - drawRect.top) / 2, Path.Direction.CW);
            outlinePaint.setColor(highlightColor);

            if (isClipPathSupported(canvas)) {
                //模糊区域绘制
                if (showThirds) {
                    canvas.clipPath(thirdPath, Region.Op.DIFFERENCE);
                } else if (showCircle) {
            ...
            canvas.restore();
            if (showCircle) {
                canvas.drawPath(circlePath, outlinePaint);
            }
            canvas.drawPath(thirdPath, outlinePaint);
